\section{Cross-testbed transfer and within-HAI LOPO}
...
\subsection{Finding 3.1 --- source calibration collapses to the class-prior baseline}
...
\subsection{Finding 3.2 --- target calibration reveals a classical-vs-SOTA split}
...
\subsubsection{Why only the windowed models transfer}
...
\subsection{Finding 3.3 --- within-HAI LOPO isolates feature shift}
...
\subsubsection{P2 drop produces targeted blindness}
...
\subsubsection{P1 drop produces global degradation}
...
\subsubsection{P3 and P4 drop is mild and sometimes helpful}
...
\subsubsection{No consistent SOTA advantage under LOPO}
...
\subsection{Cross-reading: sensor-level transfer ceilings}
...
\subsection{External-validation pipeline}
\label{sec:external-validation}

The transfer study above is closed over the two datasets chosen in Chapter~\ref{ch:method}.  To make the trained detectors directly inspectable, we ship a small external-validation pipeline as the thesis demo artifact: each run in \file{experiments/configs/} can opt into saving its fitted model, scaler, and threshold as a self-contained on-disk artifact, and a thin CLI (\code{python -m scripts.score\_external}) plus FastAPI app at \file{app/} apply that artifact to a Morris gas-pipeline ARFF or CSV.  The saved artifact reproduces the original summary-parquet row to floating-point tolerance on the held-out test split, which serves as both a reproducibility gate (no drift between train-time metrics and post-hoc scoring) and an entry point for a reader to score new captures against our detectors without retraining.  The web app exposes two input paths: an upload form for the visitor's own file, and a ``use bundled sample'' selector backed by two test-split slices we ship under \file{data/demo/} (one HAI 21.03, one Morris gas-pipeline) so the canonical thesis numbers can be reproduced in a browser without acquiring the source datasets.
