...
\subsection{What the three findings add up to}

Taken together:
\begin{itemize}
  \item \textbf{Detection (Section~\ref{ch:results-detection}).}  SOTA windowed models beat classical ones on same-dataset eTaPR; rankings change with metric choice; PA-F1 inflation is real.
  \item \textbf{Transfer (Section~\ref{ch:results-transfer}).}  Source-calibrated thresholds don't transfer; target-calibrated thresholds reveal a genuine classical/SOTA split; within-HAI LOPO shows feature-specific blindness.
  \item \textbf{Attribution (Section~\ref{ch:results-attribution}).}  Dense AE beats SOTA on localization; attention rollout adds nothing.
\end{itemize}
...
USAD eTaPR $0.009$ on Morris (Section~\ref{ch:results-detection}) is not cherry-picked; it is what the model's scores produce.  The collapse is reported as a finding about USAD's assumptions on discrete data, not hidden.

\paragraph{``Only one seed for Phase 4.''}
Acknowledged; listed as a limitation.  Phase 1--2 multi-seed numbers showed eTaPR variance $\leq 0.02$ across seeds for the classical models; attribution seed-variance is similar in the multi-seed runs reported in Section~\ref{ch:results-attribution}.  The P1/P2/P3 ordering is robust to that magnitude of noise.

\paragraph{``Process-level attribution is weak.''}
Yes; HAI 21.03 does not publish per-sensor attack targets in machine-readable form.  Section~\ref{ch:results-attribution}, \S``Limitations'' lists this as the primary weakness and points at HAI 22.04 as a follow-up.
...
\paragraph{Threshold-transfer semantics in the external-validation tool.}
The pipeline described in \S\ref{sec:external-validation} intentionally persists both the fitted model and the source-derived threshold, even though the source threshold is rarely calibrated for a new deployment.  This is a methodological choice: freezing the threshold keeps the artifact bit-reproducible against the run that produced it (the ``does my tool match my thesis number'' check), while the CLI and web app additionally accept a \code{target\_val\_percentile} recalibration matching the transfer protocol of \S\ref{sec:finding-tgt-cal}.  Users scoring an unseen dataset should read source-threshold F1 as a reproducibility check and target-calibrated F1 as the operational number; reporting only one would repeat the \code{src-cal} vs.\ \code{tgt-cal} confusion this thesis flags.

\subsection{What we deliberately did not test}
...
\begin{enumerate}
  \item \textbf{HAI 22.04 per-sensor attribution.}  22.04's README contains per-attack target-sensor descriptions that can be parsed into a ground-truth mapping.  Running the Section~\ref{ch:results-attribution} eval against that would upgrade the localization claim from process-level to sensor-level.
  \item \textbf{Multi-seed attribution} across all six models to confirm or refute the LSTM-AE $\equiv$ USAD coincidence (Section~\ref{ch:results-attribution}, Finding 4.3).
